# countpow.ps1
# By: Tim Bentley
# This will deliver a count of powered on servers on a cluster
...
$multiLineText = Read-MultiLineInputBoxDialog -Message "Enter Cluster names..." -WindowTitle "Count VMs" -DefaultText $null

$hostList = $multiLineText.Split("`r`n") | ?{ $_ }
$Clusters = @()
ForEach ($Cluster in $hostList) {
    $Clusters += $Cluster
}
